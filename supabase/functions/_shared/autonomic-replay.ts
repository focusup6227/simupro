// Deno / Edge vendored autonomic replay. Canonical logic lives in
// src/lib/physiology/autonomic-engine.ts — keep in sync manually.
import type { PathophysiologyAxes, VitalDeltas, VitalAxis } from "./pk-replay.ts";

const VITAL_AXES: VitalAxis[] = ["hr", "sBp", "dBp", "rr", "spo2"];

export function zeroDeltas(): VitalDeltas {
  return { hr: 0, sBp: 0, dBp: 0, rr: 0, spo2: 0 };
}

export function mergeAutonomicWithPkDeltas(a: VitalDeltas, b: VitalDeltas): VitalDeltas {
...
type ScenarioVitals = { hr: string; bp: string; rr: string; spo2: string; gcs: string };
...
export function mergeVitalsForDisplay(baseline: ScenarioVitals, deltas: VitalDeltas): ScenarioVitals {
  ...
  if (bp) {
    ...
    const dia = Math.max(0, Math.min(sys, Math.round(Number.parseInt(bp[2]!, 10) + deltas.dBp)));
    out.bp = `${sys}/${dia}`;
  }
...
export type AutonomicEventKind =
  | "fluid_bolus"
  | "bleed_rate_set"
  | "bleed_rate_change"
...
export type AutonomicProfile = {
  ...
  baselineBleedRateMlPerMin?: number;
  baselineDistributiveToneFactor?: number;
  baselineMapMmHg?: number;
  initialPulmonaryEdemaSeverity?: number;
  initialTensionPneumoSeverity?: number;
  initialDecompensationPhase?: DecompensationPhase;
};
...
export function defaultAutonomicState(
  ...
  return {
    ...
    distributiveToneFactor: clamp01(profile?.baselineDistributiveToneFactor ?? 1),
    sympatheticDrive: 0,
    baroreflexErrorIntegral: 0,
    workOfBreathing: 0,
    oxygenationDriveDeficit: 0,
    supplementalO2Boost: 0,
    pulmonaryEdemaSeverity: clamp01(profile?.initialPulmonaryEdemaSeverity ?? 0),
    tensionPneumoSeverity: clamp01(profile?.initialTensionPneumoSeverity ?? 0),
    decompensationPhase: profile?.initialDecompensationPhase ?? "baseline",
  ...
  for (const ev of events) {
    ...
    switch (ev.kind) {
      ...
      case "bleed_rate_set": {
        ...
        if (Number.isFinite(r) && r >= 0) st.currentBleedRateMlPerMin = r;
        break;
      }
      case "bleed_rate_change": {
        const d = Number(p.deltaMlPerMin);
        if (Number.isFinite(d)) {
          st.currentBleedRateMlPerMin = Math.max(0, st.currentBleedRateMlPerMin + d);
        }
        break;
      }
      case "distributive_tone_set": {
        const f = Number(p.factor);
        if (Number.isFinite(f)) st.distributiveToneFactor = Math.max(0.1, Math.min(1, f));
        break;
      }
      case "oxygen_change": {
        const lpm = Number(p.lpm);
        if (Number.isFinite(lpm) && lpm >= 0) st.supplementalO2Boost = Math.min(1, lpm / 15);
        break;
      }
      ...
      case "tension_pneumo_start": {
        ...
        if (Number.isFinite(s)) st.tensionPneumoSeverity = clamp01(s);
        else st.tensionPneumoSeverity = Math.min(1, st.tensionPneumoSeverity + 0.35);
        break;
      }
      case "cpap_started":
        st.cpapActive = true;
...
function updateDecompPhase(
  ...
  const hypo = mapObs != null && mapObs < 65 + 10 * clamp01(axes.hemodynamicReserve);
  ...
  if (hypo || hypox) {
    ...
    else if (phase === "decompensating" && inflammatory < 0.35) next = "compensated";
  }
  if (inflammatory > 0.55) {
    if (next === "baseline" || next === "compensated") next = "decompensating";
  }
  return next;
}
...
export function tickAutonomic(
  ...
  dtSec: number,
  axes: PathophysiologyAxes,
  observedMergedPrev: ScenarioVitals,
  eventsAtSecond: readonly AutonomicEvent[],
  simSec: number,
  cumulativeBefore: VitalDeltas,
): TickAutonomicResult {
  let st: AutonomicState = applyAutonomicEventsToState(state, eventsAtSecond, simSec);
  ...
  const volRatio = st.volumeBaselineMl > 0 ? st.intravascularVolumeMl / st.volumeBaselineMl : 1;
  ...
  const mapTarget = 85;
  const err = MAP_forReflex > 0 ? (mapTarget - MAP_forReflex) / Math.max(40, mapTarget) : 0;
  st.baroreflexErrorIntegral += err * dtSec * 0.08;
  const integralGain = 0.06 * Math.max(0.15, axes.baroreceptorSensitivity);
  const symTargetRaw = err * 1.25 + st.baroreflexErrorIntegral * integralGain;
  const symTarget = Math.max(-0.35, Math.min(1.15, symTargetRaw * axes.baroreceptorSensitivity));

  const tau = 0.22 / Math.max(0.15, axes.adrenergicReserve);
  st.sympatheticDrive += (symTarget - st.sympatheticDrive) * Math.min(1, dtSec / tau);

  const emaxHr = 38;
  const dHr = st.sympatheticDrive * emaxHr * clamp01(axes.adrenergicReserve);

  const emaxBp = 28;
  const dBpSys = st.sympatheticDrive * emaxBp * clamp01(axes.vascularTone);
  const dBpDia = dBpSys * 0.55;

  let spo2ForChem = SpO2_obs ?? 94;
  const hypoxicErr = (92 - spo2ForChem) / 14;
  st.oxygenationDriveDeficit = Math.max(-0.4, Math.min(1, hypoxicErr * clamp01(axes.oxygenAffinity)));
  st.oxygenationDriveDeficit *= 1 - clamp01(st.supplementalO2Boost) * 0.55;
  if (st.airwaySecured) st.oxygenationDriveDeficit *= 0.65;

  let dRr = st.oxygenationDriveDeficit * 7;
  st.workOfBreathing += Math.abs(st.oxygenationDriveDeficit) * 0.018 * dtSec;
  if (st.cpapActive) {
    st.workOfBreathing = Math.max(0, st.workOfBreathing - 0.012 * dtSec);
    dRr -= 1.5;
  }
  if (st.workOfBreathing > 0.85) dRr *= 0.45;
  ...
  if (
    ...
    const excess = (st.intravascularVolumeMl - st.volumeBaselineMl) / Math.max(1, st.volumeBaselineMl);
    const rate =
      0.014 *
  ...
  let dSpo2 = 0;
  if (st.pulmonaryEdemaSeverity >= 0.22) dSpo2 -= 18 * st.pulmonaryEdemaSeverity;
  if (st.pulmonaryEdemaSeverity >= 0.55) dRr += 5;

  const pneu = st.tensionPneumoSeverity;
  const dBpPneu = pneu > 0 ? -22 * pneu : 0;
  dSpo2 -= 12 * pneu;

  st.decompensationPhase = updateDecompPhase(st.decompensationPhase, MAP_obs, spo2ForChem, st.sympatheticDrive, axes);

  const crashingThresh = 38 + 5 * clamp01(axes.hemodynamicReserve);
  if (st.decompensationPhase === "crashing" && MAP_obs != null && MAP_obs < crashingThresh) {
    st.crashingSecondsAccumulated += dtSec;
  } else {
    st.crashingSecondsAccumulated = Math.max(0, st.crashingSecondsAccumulated - dtSec * 0.5);
  }
  if (st.crashingSecondsAccumulated >= 28) st.decompensationPhase = "arrested";

  const step = zeroDeltas();
  step.hr = dHr;
  ...
  return { state: st, deltasForStep: step, cumulativeDeltas: cumulative };
}
...
export function replayAutonomicAt(
  ...
  if (throughSimSec < 0) {
    return { state, cumulativeDeltas: cumulative, decompensationPhase: state.decompensationPhase };
  }
  ...
  for (let sec = 0; sec <= throughSimSec; sec++) {
    ...
    const res = tickAutonomic(state, 1, axes, observed, evs, sec, cumulative);
    state = res.state;
...
export type SupabaseAutonomicRow = {
  id: string;
  session_id: string;
  user_id: string;
  kind: string;
...
export function rowToAutonomicEvent(row: SupabaseAutonomicRow): AutonomicEvent {
  ...
  if (!isKind(kindStr)) {
    return {
      ...
      simSeconds: typeof row.sim_seconds === "number" ? row.sim_seconds : 0,
