...
type ChatterLike = string | readonly string[];
...
type DelegateCategory = 'quick' | 'airway' | 'circulation' | 'meds' | 'logistics';
...
const CATEGORY_META: Record<Exclude<DelegateCategory, 'quick'>, { label: string }> = {
...
/**
 * Catalog of partner-delegated actions. Each entry describes:
 *  - what the partner says (`chatter`, optionally rotated through multiple lines)
 *  - what gets logged for grading (`assessmentDetail`)
 *  - which seeded interventions count as treatments (`treatmentIds`)
 *  - pediatric overrides where dose / cadence diverges enough to matter
 */
const DELEGATE_CATALOG: DelegateEntry[] = [
  // -------------------- Quick --------------------
  {
    key: 'manual-bp',
    category: 'quick',
