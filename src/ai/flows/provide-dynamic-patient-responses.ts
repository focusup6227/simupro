...
- **Stressor plausibility:** \`stressors\` model **autonomous disease evolution the deterministic engine doesn't already know** (a rebleed, sepsis worsening, new bronchospasm) — they are **not** a penalty mechanism for the learner's mistakes. Only emit them for complications consistent with the scenario's pathology. The harm from a **mechanical or out-of-scope error** (e.g. CPR or another intervention on a perfusing patient) belongs in \`patientResponse\`/\`conditionChange\` as pain, injury, or distress — do **not** emit pathophysiology \`stressors\` for it, and never invent unrelated pathology (e.g. no spontaneous tension pneumothorax on an uncomplicated MI).
